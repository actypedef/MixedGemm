...
def add_w4a16_linear_weight(network, shape, name_prefix):
    ...
    dequantize_layer.axis = 1 # Per-channel (axis=1 for shape [K, N])
...
def create_dummy_weights(shape, dtype=np.float16):
    """创建一个包含随机数据的 trt.Weights 对象 (用于无需关心数值的权重，如RMSNorm的gamma)"""
...
def build_decoder_layer_engine():
    ...
    # --- [修正] ---
    # 在 STRONGLY_TYPED 网络中，实现W4A16不需要设置任何全局精度标志。
    # TensorRT会通过 "Constant(INT4) -> Dequantize -> MatMul" 的模式
    # 自动识别并融合为高效的权重仅量化内核。
    # 因此，删除了 config.set_flag(trt.BuilderFlag.INT8) 等相关代码。
    ...
    print("Building Attention Block with W4A16 Weights and RoPE...")
    ...
    print("Building FFN Block with W4A16 Weights...")
    ffn_gate_w = add_w4a16_linear_weight(network, (HIDDEN_SIZE, FFN_HIDDEN_SIZE), "ffn_gate")
    ffn_up_w = add_w4a16_linear_weight(network, (HIDDEN_SIZE, FFN_HIDDEN_SIZE), "ffn_up")
    ffn_down_w = add_w4a16_linear_weight(network, (FFN_HIDDEN_SIZE, HIDDEN_SIZE), "ffn_down")
...
def benchmark(engine_plan):
    ...
    print(f"Precision: W4A16 (Weights: INT4, Activations: FP16)")
