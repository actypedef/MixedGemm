...
def add_rope(network, input_tensor, cos_cache, sin_cache, num_heads):
    """在网络中添加 RoPE 层"""
    # input_tensor 形状: (B, H, S, D)
    head_dim_half = HEAD_DIM // 2

    # [修正] 在 add_slice 中使用显式维度而不是-1，以避免形状推断错误。
    # 这能解决 `inherently negative length` 的 TRT 错误。
    ...
    concat_layer.axis = 3 # 沿 head_dim 维度 (D) 拼接
...
def add_rmsnorm(network, input_tensor, weight_shape):
    ...
    # 修复 len() 错误将依赖于上游张量形状的正确性。
    # 这里的代码逻辑是正确的，只要 input_tensor.shape 有效。
...
def build_decoder_layer_engine():
    ...
    quantize_scale = network.add_constant((1,), trt.Weights(np.array([1.0], dtype=np.float32))).get_output(0)
    q_layer = network.add_quantize(input_tensor, quantize_scale, trt.fp8)
    fp8_input = q_layer.get_output(0)
    dequantize_layer = network.add_dequantize(fp8_input, quantize_scale, trt.float16)
    fp16_input = dequantize_layer.get_output(0)

    normed_input = add_rmsnorm(network, fp16_input, (HIDDEN_SIZE,))
    ...
    residual1 = network.add_elementwise(fp16_input, attention_output, trt.ElementWiseOperation.SUM).get_output(0)
...
def benchmark(engine_plan):
    ...
    print(f"Precision: FP8 (via STRONGLY_TYPED network definition)")
